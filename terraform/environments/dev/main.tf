terraform {
  ...
  # backend "s3" {
  #   bucket         = "opengnosis-terraform-state-dev"
...
module "postgresql" {
  ...
  allocated_storage   = 50  # Smaller for dev
  ...
  backup_retention_days = 7  # Shorter for dev
...
module "redis" {
  ...
  subnet_ids                  = module.networking.private_subnet_ids
  allowed_cidr_blocks         = [module.networking.vpc_cidr]
  automatic_failover_enabled  = false  # Disabled for dev
...
module "elasticsearch" {
  ...
  dedicated_master_enabled = false  # Disabled for dev
